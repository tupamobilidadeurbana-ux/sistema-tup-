...
const Manager: React.FC<ManagerProps> = ({ 
  ...
  const filteredBilling = useMemo(() => {
    ...
    let list = acquisitions.filter(a => !!a.paymentDate);
    if (billingStartDate) list = list.filter(a => a.paymentDate && a.paymentDate >= billingStartDate);
    if (billingEndDate) list = list.filter(a => a.paymentDate && a.paymentDate <= billingEndDate);
    ...
    return list;
  }, [acquisitions, userLocation, billingStartDate, billingEndDate]);
  ...
  const focusOnClient = (acq: Acquisition) => {
    if (acq.location && mapRef.current) {
      mapRef.current.setView([acq.location.latitude, acq.location.longitude], 16);
      const marker = markersRef.current.get(acq.id);
      if (marker) marker.openPopup();
      window.scrollTo({ top: 0, behavior: 'smooth' });
    }
  };

  const openInGoogleMaps = (acq: Acquisition | Partial<Acquisition>) => {
    if (acq.location) {
  ...
  const handleUpdateCategory = async (id: string) => {
    if (editingCategoryName.trim()) {
      setIsSubmitting(true);
      try { await onUpdateCategory(id, editingCategoryName); setEditingCategoryId(null); }
      catch (err: any) { alert(err.message); }
      finally { setIsSubmitting(false); }
    }
  };
                    ...
                    <div key={c.id} className="group relative">
                      {editingCategoryId === c.id ? (
                        <div className="flex gap-2">
                          ...
                          <button onClick={() => handleUpdateCategory(c.id)} className="p-3 bg-blue-600 text-white rounded-xl shadow-lg hover:bg-blue-700 transition-all"><svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2.5" d="M5 13l4 4L19 7"/></svg></button>
                ...
                <form onSubmit={handleSiteSubmit} className="grid grid-cols-1 md:grid-cols-2 gap-8">
                  ...
                  <div className="md:col-span-2">
                    ...
                    <div onClick={() => siteFileRef.current?.click()} className="w-full h-64 bg-slate-50 border-2 border-dashed border-slate-200 rounded-[2.5rem] flex flex-col items-center justify-center cursor-pointer hover:bg-slate-100 hover:border-blue-300 transition-all overflow-hidden relative group">
                      {siteForm.mediaUrl ? (
                        <>
                          {siteForm.mediaType === 'video' ? <video src={siteForm.mediaUrl} className="w-full h-full object-cover" autoPlay muted loop /> : <img src={siteForm.mediaUrl} className="w-full h-full object-cover" />}
                          <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 flex items-center justify-center transition-opacity backdrop-blur-sm">
                    ...
                    <div key={s.id} className="bg-white p-6 rounded-[2.5rem] border border-slate-100 flex items-center justify-between group shadow-sm hover:shadow-xl transition-all">
                      <div className="flex items-center gap-5">
                        <div className="w-16 h-16 rounded-2xl bg-slate-100 overflow-hidden shadow-inner">
                          {s.mediaType === 'video' ? <video src={s.mediaUrl} className="w-full h-full object-cover" muted /> : <img src={s.mediaUrl} className="w-full h-full object-cover" />}
                        </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
             <section className={`p-10 rounded-[3rem] shadow-sm border h-fit transition-all duration-500 ${editingConsultantId ? 'bg-blue-50 border-blue-200 ring-4 ring-blue-500/10' : 'bg-white border-slate-100'}`}>
                ...
                <form onSubmit={handleConsultantSubmit} className="space-y-6">
                  ...
                  <div className="space-y-4">
                    <label className="block text-[9px] font-black text-slate-400 uppercase tracking-widest px-2">URL da Foto</label>
                    <input type="text" className="w-full px-8 py-5 bg-white border-2 border-slate-100 rounded-2xl outline-none focus:border-blue-600 font-bold" value={consultantForm.photoUrl} onChange={(e) => setConsultantForm({ ...consultantForm, photoUrl: e.target.value })} />
                  </div>
                ...
                <div key={acq.id} className="p-8 md:p-12 bg-white rounded-[4rem] border border-slate-100 shadow-2xl flex flex-col lg:flex-row justify-between gap-10">
                  <div className="flex-1 space-y-6">
                    ...
                    <div className="bg-slate-50 p-6 rounded-3xl border border-slate-100 flex flex-wrap gap-10">
                        ...
                        <div><p className="text-[10px] font-black uppercase tracking-widest text-slate-400 mb-1">Vencimento Ref.</p><p className="text-xl font-bold text-slate-700">{acq.paymentDate ? new Date(acq.paymentDate).toLocaleDateString('pt-BR') : 'Hoje'}</p></div>
                    </div>
                  </div>
          ...
          <div className="space-y-12">
            <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
              <div>
                ...
                <p className="text-xs font-bold text-slate-400 uppercase tracking-widest mt-2">Navegação inteligente por proximidade de parcelas</p>
              </div>
              <div className="flex flex-wrap gap-4 items-end">
                <div className="space-y-2">
                  <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest px-2">Data Início</label>
                  <input type="date" className="px-6 py-4 bg-white border border-slate-200 rounded-2xl text-xs font-bold outline-none" value={billingStartDate} onChange={(e) => setBillingStartDate(e.target.value)} />
                </div>
                <div className="space-y-2">
                  <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest px-2">Data Fim</label>
                  <input type="date" className="px-6 py-4 bg-white border border-slate-200 rounded-2xl text-xs font-bold outline-none" value={billingEndDate} onChange={(e) => setBillingEndDate(e.target.value)} />
                </div>
                <button onClick={handleNextClient} className="bg-blue-600 text-white px-8 py-4 rounded-2xl font-black text-[10px] uppercase tracking-[0.2em] shadow-xl hover:bg-slate-900 transition-all flex items-center gap-3 h-fit">Próximo Cliente <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="3" d="M9 5l7 7-7 7"/></svg></button>
              </div>
            </div>
            ...
            <div className="relative group">
               ...
               <div className="absolute top-4 right-4 z-[1000] bg-white/90 backdrop-blur-md p-4 rounded-2xl border border-slate-200 shadow-xl">
                  <p className="text-[10px] font-black uppercase text-slate-400 tracking-widest mb-1">Parcelas Filtradas</p>
                 ...
                 <table className="w-full text-left">
                   <thead>
                     <tr className="bg-slate-50 border-b border-slate-100">
                       ...
                       <th className="px-8 py-6 text-[10px] font-black uppercase tracking-[0.2em] text-slate-400">Consultor</th>
                       <th className="px-8 py-6 text-[10px] font-black uppercase tracking-[0.2em] text-slate-400">Parcela Atual</th>
                   ...
                   <tbody className="divide-y divide-slate-50">
                     {filteredBilling.map((acq, index) => (
                       <tr key={acq.id} className={`hover:bg-blue-50/30 transition-colors ${focusedClientIndex === index ? 'bg-blue-50/50' : ''}`}>
                         ...
                         <td className="px-8 py-6"><span className="text-xs font-bold text-slate-600">{consultants.find(c => c.id === acq.consultantId)?.name}</span></td>
                         <td className="px-8 py-6"><span className="font-black text-slate-900">R$ {acq.installmentValue?.toLocaleString('pt-BR')}</span></td>
                         <td className="px-8 py-6"><span className="text-xs font-bold text-slate-500">{acq.paymentDate ? new Date(acq.paymentDate).toLocaleDateString('pt-BR') : 'N/A'}</span></td>
                     ...
                     {filteredBilling.length === 0 && (
                       <tr>
                         <td colSpan={7} className="px-8 py-20 text-center text-slate-400 font-bold uppercase tracking-widest text-xs">Utilize o filtro de data para visualizar as cobranças no mapa</td>
                       </tr>
                     )}
                   </tbody>
                 </table>
